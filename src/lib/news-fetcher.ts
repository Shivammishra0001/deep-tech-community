    ...
    try {
      ...
      for (const item of parsedItems.slice(0, 5)) { // Process top 5 recent items per feed
        ...
        const newItem: NewsItem = {
          ...
          image_url: item.imageUrl || imageConfig.image_url,
          image_source: item.imageUrl ? `${src.name} (Official Feed Enclosure)` : imageConfig.image_source,
          license: item.imageUrl ? "Official Feed Enclosure (Editorial / Attribution)" : imageConfig.license,
